...
getGenesInLoci.pl -d CLdb.sqlite > gene_info.txt

=head2 Direct loading to CRISPR database

getGenesInLoci.pl -d CLdb.sqlite | CLdb_loadGenes.pl -d CLdb.sqlite

=head2 Refining query to just the 'I-B' subtype

getGenesInLoci.pl -d CLdb.sqlite -sub I-B

=head2 Refining query to just taxon '6666666.40253'

getGenesInLoci.pl -d CLdb.sqlite -taxon_id 6666666.40253

=head2 Write out all entries (new & old; on conflict: old entry is written)

getGenesInLoci.pl -d CLdb.sqlite -a

=head2 Write out all conflicting entries 

getGenesInLoci.pl -d CLdb.sqlite -c
