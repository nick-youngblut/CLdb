...
comparison_make.pl -- making comparisons table for plotting

=head1 SYNOPSIS

comparison_make.pl [flags] < dna_segs.txt > compare.txt
...
of spacer sequences (hclusterArrays.pl must
be run prior), or by pairwise spacer blasts 
(spacerPairwiseBlast.pl must be run prior).
...
comparison_make.pl -da CLdb.sqlite  < dna_segs.txt > compare.txt

=head2 Just comparisons among spacers (using clustering info)

comparison_make.pl -da CLdb.sqlite -s cluster < dna_segs.txt > compare.txt

=head2 Comparisons among spacers and genes

comparison_make.pl -da CLdb.sqlite -I ITEP_database.sqlite < dna_segs.txt > compare.txt
...
if( exists $dna_segs_r->{"gene"} && $dbh_ITEP ){
	...
	get_gene_start_end($dbh, $dna_segs_r, $dna_segs_order_r, $blastp_res_r, \%compare);
	}
