...
summary.pl -- summary stats on CLdb

=head1 SYNOPSIS

summary.pl [flags] > summary.txt
...
if ( exists $table_list_r->{"loci"} && exists $table_list_r->{"spacers"} ){
	...
	sum_spacers_DR_clusters($dbh, "spacer", $group_by_r, \@cutoff);
	}	
...
if ( exists $table_list_r->{"loci"} && exists $table_list_r->{"drs"} ){ 
	...
	sum_spacers_DR_clusters($dbh, "DR", $group_by_r, \@cutoff);
	}	
...
sub sum_loci{
	...
	my @select;
	push @select, @$group_by_r if @$group_by_r;
	push @select, qw/a.CAS_status a.array_status/;
	my $select = join(",", @select);
