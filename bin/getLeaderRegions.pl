...
getLeaderRegion.pl -- getting leader region sequences

=head1 SYNOPSIS

getLeaderRegion.pl [flags] > possible_leaders.fna
...
CLdb --perldoc -- getLeaderRegions.pl
...
groupArrayElements.pl has to be run on the database 
...
Load the leader region using loadLeaders.pl
...
# if no $degen_check; leader could be on either strand
  ...
  foreach my $locus (keys %$array_se_r){		# each CRISPR locus
	...
	unless $$ret[0];
      die " ERROR: no 'repeat_group' entries found!\n Run groupArrayElements.pl before this script!\n\n"
