...
tree_prune.pl -- prune tree to just taxa in provided dna_segs or xlims table.

=head1 SYNOPSIS

=head2 Ordering 'dna_segs' table

tree_prune.pl [flags] -d dna_segs.txt < tree_file > tree_file_prn

=head2 Ordering 'xlims' table

tree_prune.pl [flags] -x xlims.txt < tree_file > tree_file_prn
...
tree_prune.pl -d dna_segs.txt < tree.nwk > tree_prn.nwkk

=head3 Pruning by an xlims table

tree_prune.pl -x xlims.txt < tree.nwk > tree_prn.nwk
