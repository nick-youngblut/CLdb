...
loadLeaders.pl -- adding/updating leader entries in to CRISPR_db

=head1 SYNOPSIS

loadLeaders.pl [flags] leader_aligned.fasta
...
CLdb --perldoc -- loadLeaders

=head1 DESCRIPTION

After determining the actual leader region by aligning
approximate leader regions written by getLeaderRegions.pl,
...
# loading fasta #
  ...
  while(<IN>){
    ...
    if(/>.+/){
      ...
      my @line = split /[>|]/;
      die "ERROR: leader sequence: '$_' does not have getLeaderRegions.pl formatting!\n"
