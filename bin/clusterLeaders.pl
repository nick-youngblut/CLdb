...
clusterLeaders.pl -- cluster leader sequences & adding to CLdb

=head1 SYNOPSIS

clusterLeaders.pl [flags] 
...
Directory where intermediate files are written. [$HOME/grouping/]
...
CLdb --perldoc -- clusterLeaders.pl
...
are written to '$HOME/grouping/' by default.

=head2 Requires:

cd-hit-est, leader2fasta.pl
...
# calling leader2fasta to make fasta sequences of array elements (no gaps) #
	...
	my $outfile = "$dir/leaders.fna"; 
	`leader2fasta.pl -d $database_file -g > $outfile`;
