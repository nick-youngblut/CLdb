...
CLdb_arrayBlastGetProto.pl -- getting protospacer from blast srl (assuming no crDNA info)

=head1 SYNOPSIS

CLdb_arrayBlastGetProto.pl [flags] < blast_hits.srl > spacer_blast_proto.txt
...
Run CLdb_arrayBlastAddProto.pl 
...
Just blast your sequences, run CLdb_arrayBlastAddProto.pl,
then this script.

The output is a tab-delimited table (with header).
Convert this to a fasta with 'CLdb_table2fasta.pl'
