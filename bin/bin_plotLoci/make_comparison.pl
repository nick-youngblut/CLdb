...
make_comparison -- making comparisons table for plotting

=head1 SYNOPSIS

make_comparison [flags] < dna_segs.txt > compare.txt
...
print STDERR "...WARNING: no ITEP db provided ('-i')!",
  " No blastp information will be included!\n"
  unless $ITEP_sqlite;
...
if( exists $dna_segs_r->{"gene"} && $dbh_ITEP ){
  my $blastp_res_r = get_ITEP_blastp($dbh_ITEP, 
				     $dna_segs_r, 
				     $dna_segs_order_r, 
				     $blastp_cutoff);
  get_gene_start_end($dbh, 
		     $dna_segs_r, 
		     $dna_segs_order_r, 
		     $blastp_res_r, 
		     \%compare);
...
# writing out compare table #
  ...
  # header #
  print join("\t", qw/start1 end1 start2 end2 col
		      dna_segs_id1 dna_segs_id2 feat
		      feat_id sfeat_id/), "\n";
...
";
  ...
  for my $i (0..($#$dna_segs_order_r-1)){
    ...
    # sanity check 
    unless( defined $locus_id1 && 
	    exists $dna_segs_r->{gene}{$dna_seg_id1}{$locus_id1} ){
